 ...
 *   egress                 : Egress settings controlling what traffic is diverted through the VPC Access Connector resource ('all' || 'private-ranges-only', optional, default 'private-ranges-only')
...
def call(Map config) {
    ...
    if (!['all', 'internal-only', 'internal-and-gclb', ''].contains(config.ingress)) {
        error "Invalid egress specified. Must be 'all', 'internal-only', 'internal-and-gclb'."
    }

    if (!['all', 'private-ranges-only', ''].contains(config.ingress)) {
        error "Invalid egress specified. Must be 'all', 'private-ranges-only'."
    }
...
def deployFunction(Map config) {
    ...
    if (config.serviceAccountEmail != '') {
        deployCommand += " --service-account ${config.serviceAccountEmail} \\"
    }

    deployCommand += """
        --source "${env.WORKSPACE}/${config.functionName}" \\
        ...
        --ingress-settings ${config.ingress != '' ? config.ingress : 'all'} \\
        --egress-settings ${config.egress != '' ? config.egress : 'private-ranges-only'} \\
