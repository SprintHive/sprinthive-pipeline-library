        ...
        container('terraform') {
            ...
            for (workspace in targetWorkspaces) {
                stage("Terraform Plan: ${workspace}") {
                    ...
                    script: "(cd ${config.TF_DIRECTORY} && terraform init && terraform workspace select ${workspace} && terraform plan -out plans/${workspace}.tfplan ${targetArguments.join(' ')}) | tee -a logs/plans/${workspace}.log"
